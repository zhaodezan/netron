        ...
        return keras.Metadata.open(context).then((metadata) => {
            ...
            if (stream.length > signature.length && stream.peek(signature.length).every((value, index) => value === signature[index])) {
                return context.require('./hdf5').then((hdf5) => {
                    ...
                    const model_config_json = rootGroup.attributes.get('model_config');
                    const layer_names = load_attributes_from_hdf5_group(rootGroup, 'layer_names');
                    if (model_config_json || layer_names) {
                        ...
                        if (!model_weights_group && rootGroup.attributes.get('layer_names')) {
                        ...
                        if (Array.from(rootGroup.groups.values()).every((group) => group.attributes.size === 0 && group.groups.length == 0 && group.value !== null)) {
                        ...
                        else if (Array.from(rootGroup.groups.values()).every((group) => group.value === null && Array.from(group.attributes.keys()).filter((key) => !tensorKeys.has(key)).length === 0 && Array.from(group.groups.values()).every((variable) => Object.keys(variable.attributes).length === 0 && variable.value !== null))) {
                            for (const group of rootGroup.groups.values()) {
                            ...
                            const walk = function(group) {
                                ...
                                else if (Array.from(group.attributes.keys()).filter((key) => !subKeys.has(key)).length === 0 && group.value !== null && group.groups.size === 0) {
                                    const variable = group.value;
